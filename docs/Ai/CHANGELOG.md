...
## 2026-09-03
- **Fixed a severe touch-layout break: the keypad was being pushed off-screen on phones.** `.main-grid` sets `align-items: start` for the desktop CSS *grid*; the touch override switches it to `display: flex; flex-direction: column`, where that same value applies to the **cross (horizontal) axis** and made `.play-col` size to fit-content instead of the viewport. The widest descendant is the touch diff strip (`width: max-content`, one ~427px item per enabled op, driven by the nowrap "Locked: Master this level to unlock its challenges." chip), so with several ops enabled the column grew to ~1290px — 903px off-screen on a 393px phone, silently, because `body` is `overflow-x: hidden`. Consequences: keys `3 6 9 0 * / ^ - .`, Tab, Enter and backspace were off-screen and not hit-testable (so no answer containing 3/6/9 could be typed and Enter could not be pressed); `.kp-diff-scroll` never scrolled because it grew instead, hiding ops past the first one or two; and the canvas ended up with a 1284px CSS box over a 1031px backing store, drawing the playfield at the wrong scale with most of it off-screen. Fixed with `align-items: stretch` (plus `min-width: 0` on `.play-col`) in the touch-only rule, which also lets the diff strip scroll as originally designed. iPad was affected too (1290px in an 810px viewport).
- Fixed the touch header: `.touch-finish` was missing from the `width: auto` link group, so "Finish" rendered inside a 22px icon circle and its text collided with the Log pill. Added it to the group, and gave `.touch-brand` `flex-wrap: wrap` + `row-gap` so the row wraps instead of clipping its last item (the `?` was cut off on an iPhone 13) — the links are `flex-shrink: 0`, so wrapping is the only non-clipping outcome.
- Added two mobile e2e regressions that would have caught both: one asserts every `.kp-key` is on-screen **and** hit-testable via `elementFromPoint` with four ops enabled (plus that `.play-col`/keypad/canvas stay within the viewport and the diff strip actually has scroll range, and that a far-column key works end to end), the other asserts no `.touch-brand` child is clipped and that `Finish` is a text pill rather than an icon circle. The previous mobile coverage missed this because it enabled a single op and only tapped first-column keys.
- **Made the touch diff strip fit the phone, so no op control is off-screen.** Fixing the column-width bug let the strip scroll as designed, but scrolling was still the wrong answer for one op: a single row measured 427px locked / 461px boss-ready against a 378px container, so the `Recap` button was **entirely** off-screen after mastering a level, with no scroll affordance to suggest anything was there. Two label habits caused it: the lock chip spelled out a whole sentence (227px -- over half the row), and every challenge pill repeated `L<n>`, which is provably redundant because `getReplayChallengeLevel()` returns either `selectedLevel` or `0`, so a visible pill's level can never differ from the level already shown in `.kp-diff-val`. Touch labels are now compacted (`Locked: Master this level to unlock its challenges.` -> `🔒 Master to unlock`, `Blitz L1` -> `Blitz`, `Mastered: 0%` -> `0%`) with the full wording preserved in `title`/`aria-label`; desktop cards keep the long form since they wrap in a vertical panel. A single op now fits with **zero** hidden pixels in both the locked (244px) and boss-ready (346px) states.
- Two or more enabled ops genuinely cannot fit one phone row, so the strip still scrolls there -- but it now advertises it: `updateDiffStripAffordance()` toggles an `is-scrollable` class when the strip actually overflows, and CSS fades the trailing edge so hidden controls read as "swipe for more" instead of as nothing at all. Covered by a mobile e2e regression that asserts nothing overflows for one op in either state, that `Recap` is visible, that no affordance is advertised when nothing is hidden, and that the fade does appear with three ops.
- Fixed the desktop header being scrolled out of view on load. The answer input sits below the canvas, so focusing it scrolled it into view — on a short window (e.g. 1280x720) the page loaded at `scrollY: 120`, pushing the whole header, including the Menu/Test Me/Login/Log nav, above the fold. All 20 `answerInput.focus()` call sites now pass `{ preventScroll: true }`; the input stays focused so typing still works immediately. Covered by a desktop e2e regression (asserts `scrollY === 0`, the top bar is on screen, and `#answer` is still the active element) that fails without the fix on chromium/firefox/webkit.
- Corrected two bugs in the screenshot harness itself, which had produced misleading review shots: `seedStats` assigns `problemStats[opKey]` directly so it needs a keyed map (an array was passed, rendering an empty accuracy grid), and `startBoss` needs the op key (calling it bare no-opped, so the "boss" shots were really just the ready gate). Touch boss mode and the stats grid were then reviewed and are correct.
- Added `tests/e2e/_platform-shots.spec.js`, an opt-in cross-platform screenshot harness (`SHOTS=1 npx playwright test _platform-shots`) that captures seven states per device profile to `/tmp/rainmath-shots`. Gated behind `SHOTS=1` so it stays out of `npm run test:e2e`.
...
- Tightened the Start/Pause/Resume, Restart, and Finish workflows. **Start** is now disabled at the empty ready gate and when Drops is 0, with matching hint text; **Restart** is disabled until there is real run/challenge/Test Me/board state to abandon; **Finish** is disabled until the current session has reportable activity, so it no longer opens an empty report. Starting Boss/Blitz/Wave/Worksheet from the ready gate now begins immediately instead of creating a locked challenge that still waits for a second Start press. The rules are covered by pure helpers plus focused e2e across practice, Test Me, and challenge modes.
- Fixed run-control state leaks across contexts. **Restart** now refreshes the controls it unlocks — clearing a challenge (or Test Me placement) no longer leaves the op chits / diff cards stuck disabled with stale challenge visuals on the board; it also clears an in-progress Test Me. **Finish** now returns to the ready/Start gate instead of leaving the Pause/Resume control reading a stale "Pause" over an idle, all-ops-off board. Root cause: `resetRunState` cleared `bossMode` (which unlocks the controls) but never re-ran `updateOpChits`/`updateControlDisplay`/`drawDrops` the way placement-start and profile-activation do, and it didn't clear `placementState`; `finishCurrentSession` never reset `hasStarted`/`isPaused`.
- Challenges (Blitz/Wave/Worksheet) now have a clear way out: a floating **"Quit ✕"** button on the play area abandons the challenge and returns to normal practice (nothing is recorded — attempts only save on completion), separate from **Finish** (which ends the whole session). Previously the only obvious exit was Finish, since the boss text HUD is intentionally hidden and the footer **Restart** reads ambiguously. The Quit button appears only during an active challenge.
- The **Start** button now gently pulses at the ready/Start gate once a problem type (or a Blitz/Wave/Worksheet) is staged, hinting that pressing Start is the next step. It stops once the run starts, and respects `prefers-reduced-motion`.

## 2026-07-05
- Added optional **Adaptive** Speed/Drops for ordinary practice. Desktop gets an Adaptive checkbox and touch gets an Auto button; when on, Speed/Drops controls are locked but still visibly update as the game adjusts pressure. The adaptive rule starts from the player's current Speed/Drops, reduces quickly on misses/wrongs/overload/full-board pressure, raises slowly after clean quick windows, persists the setting plus per-op estimates, and ignores Boss/challenge/Test Me/Breather modes. Covered by pure helper/persistence unit tests plus desktop/touch e2e.
- Curriculum tracks **v2 phase D — human-readable arithmetic level lists**. Added a generic `arithmeticLevels` descriptor that expands readable per-level operand ranges / exact pairs into generation, weighted practice, mastery universes, boss worksheets, answer-space calculations, and track max-level UI. Reworked Times Tables from the terse `timesTable` shortcut into an explicit level list: L1-L12 are the individual 1s-12s tables, and L13 is a mixed 1×1 through 12×12 review (144 facts). Stored challenge/session level normalization and boss/Test Me level advancement now preserve track levels above 10 instead of silently clamping them.
- Curriculum tracks **v2 phase C — data-fied all 11 ops**. Every operation's level definition now lives in `src/tracks/standard.js` (read via the threaded track, so alternate tracks can override): f10 (maxDigits/maxPower), factor (minN/maxN/levelOffset), round (per-level rounding-case spec tuples), reduce (per-level concept cells), pow (a cumulative rung ladder), si (prefix table + per-prefix unlock thresholds), and shapes (partial seam — the level gate `defs` + dimension bounds are data; per-shape enumeration and area/perimeter/volume formulas stay in game-core as the generation strategy). The Standard track is now a complete data representation of the game's curriculum. Behavior-preserving throughout: the byte-for-byte golden snapshot stayed green at every op (one commit each). The rendering/generation "strategy" code (how to draw a shape, format an SI conversion, factor a number) stays in game-core, selected by the data.
- Curriculum tracks **v2 — shared facts / per-track coverage** (fixes progress bleeding across tracks). A skill's per-problem fact stats (`problems`/`totals`/`recent`/`pressureTiers`) are now **universal** (knowing 7×8 is knowing it in any curriculum), while a track's **coverage** — its current level and level-indexed progress (boss clears, level advances, placement credits, blitz/challenge bests) — moved **per-track** under `skill.tracks[trackId]`. So clearing Standard mul level 2 no longer marks Times-Tables level 2 "mastered," and mastery% recomputes from the shared facts against the *active track's* universe. `PROFILE_VERSION` 3→4 with a migration that folds every existing player's progress into `tracks.standard` (nothing lost). The many bare-`skill` readers are fed a track-resolved view (`viewSkillForTrack`) so their signatures didn't change; writers use `ensureCoverage`. New e2e proves a Standard clear doesn't carry to Times Tables while facts do.
- Reorganized track data at the file level: each track is its own module (`src/tracks/standard.js`, `src/tracks/times-tables.js`) with `src/curriculum.js` as a thin registry.

## 2026-07-04
- Added **curriculum "tracks"** — data-driven level definitions, plus a second playable track (handled directly). New `src/curriculum.js` holds `TRACKS` (pure data) + `getActiveTrack(id)`. The game's original hardcoded arithmetic levels are now expressed as the `standard` track's `range` descriptors and read through an optional trailing `track = TRACKS.standard` parameter threaded through game-core (`getDifficultyRange`, `generateProblem`, `generateWeightedProblem`, `getAnswerUniverse`) and player-progress (universe/readiness chain). Behavior-preserving: a byte-for-byte golden snapshot (`tests/support/curriculum-snapshot.json`, `tests/unit/curriculum.test.js`) proves Standard reproduces today's levels exactly.
- Shipped the **Times Tables (×)** track as the visible proof: multiply-only, where level N is the N times table (N×1 … N×12), N = 1…12. New `timesTable` universe kind (statsKeys `"N,b"` stay compatible with Standard mul, so fact stats carry over). A **Curriculum** `<select>` in the Login player manager switches tracks; switching collapses the op set to `×`, redefines the mul level universe to the 12 facts, widens the level cap to 12, and fully restores Standard (all 11 ops, 10-level cap) on switch-back. New profile field `activeTrack` (additive, back-filled via spread-default; no `PROFILE_VERSION` bump). e2e covers the switch/collapse/generate/restore loop.
- Follow-ups deliberately deferred: per-track *progress* (level + stats scoped per track — MVP shares `mul.currentLevel` and clamps on switch), moving the remaining non-arithmetic ops (f10/factor/si/reduce/round/pow/shapes) into `TRACKS.standard` as data, and additional real tracks (Saxon, Math-U-See).
...
- UX pass (implemented directly, not via Codex): (1) recap Copy/Share now hand out just the `#recap=` link, not the text blob; (2) new **ready/Start gate** — after the first visit the game opens idle so toggling problem types stages them and play begins on Start (Restart returns to the ready state; test mode auto-starts so the suite is unaffected); (3) fixed the pause/restart bug where the button could stay stuck on "Resume" after Restart (the label is now derived from run state); (4) added **no ads / no tracking / no server** messaging to the welcome support box and the session-report donate line; (5) compressed the Blitz/Wave-1 endurance onset from a ~70s slow crawl to ~42s with a more engaging start speed (constants only; ramp function/tests unchanged) so it reaches real pressure faster without overwhelming.
- Added shareable Recap achievement links: the Recap popup now copies/shares a readable summary plus a tamper-checked `#recap=` link, and opening that link shows the same recap card as a read-only shared achievement view with no profile mutation.
...
- Trimmed the v2 share blob further (it decodes regardless, so the AI gag earned its keep — gone): dropped the `note` easter egg entirely, replaced each session's long internal id with its position index ("0"), and made the tamper `id` a bare checksum (dropped the `rm<timestamp>-` disguise prefix, which was never part of the hash). A realistic single-session link is now ~160 chars (full URL ~190), down from ~250–320. Tamper checksum still validated over `{v, n, r}` (verified by the tamper test); read-only/back-compat unchanged. Note: v2 links generated earlier today (with the note in the checksummed content) will now read as broken — acceptable since the format is hours old and those links were test-only.
...
## 2026-06-27
- Added a tracked `docs/Ai/FEATURE_REQUESTS/` workflow (Codex/David proposal): one committed design doc per non-trivial feature (request → design → decisions → acceptance → outcome), so design survives a fresh clone instead of living in the gitignored `DIALOGUE.md` (now reserved for terse turn coordination). Seeded the README/template and the first worked file (`answer-space-aware-overload.md`); `agents.md` points at the folder.
- Made the cannon overload **answer-space-aware** (Codex design proposal, implemented by Claude): an impossible submission now heats the cannon by a weight that scales with how guessable the board is, instead of a flat count. New pure, type-checked, unit-tested helpers in `game-core.js` — `getAnswerUniverse(op, level)` / `getDistinctAnswerCount` (arithmetic enumerated from its operand range; shapes/pow from their universes; SI from its conversions; f10/factor are large/non-guessable → cost 1) and `falseFireCost({distinctAnswerCount, visibleDistinctAnswers})` with tunable cost tiers. `registerWrongSubmission` now sums weighted heat in the window; small spaces (e.g. L1 subtraction = {0,1,2}) overload in ~2 misses while large spaces keep the old ~5. Curbs sustained brute-force guessing without a hardcoded special-case and without touching mastery stats or shields. PURPOSE + tutorial copy updated.
- Fixed two doc-accuracy issues from a Codex review: the Tutorial copy (`src/text/english.js`) now lists Powers & Roots among the own-lane operations and notes that Test Me placement (not only 100% mastery) can unlock a higher starting level; and the `ProblemStat` typedef (`src/types.js`) now documents the real flat placement fields (`placementStatus`/`placementLevel`/`placementPlacedOutThrough`/`placementSource`/`placementAt`/`placementSupersededAt`) instead of a non-existent nested `placement` object.
- Brought the cross-agent docs current so any assistant (Codex, future sessions) picks up the project state from the repo: rewrote the stale `agents.md` (commands, ES-modules-served-over-HTTP, the layered `src/` structure, version-stamping rule) and added the known test flakiness (stale :4173 server, flaky Firefox cannon test) to both `agents.md` and `TESTING.md` — previously only in Claude-private memory.
- Recorded the design intent in `ARCHITECTURE.md` (layered by testability/coupling: pure logic → game-core; profile → player-progress; self-contained runtime → src/engine; views → src/popups; the irreducibly-coupled real-time engine stays in script.js on purpose). Concludes the structural refactor: the codebase is now ES modules + a typed/tested core (enforced in CI) + explicit shared `state` + extracted subsystems, with the cohesive engine kept whole and backed by the test/type net.
- Decoupling step 1 (safe): moved the engine-state predicates (`isBossActive`, `isPlacementActive`, `isPlacementDrop`, `isControlLocked`, `isBossStunned`) out of the boss section into `src/engine/predicates.js`. They are cheap reads of `state` used across the whole codebase, so homing them neutrally stops the boss code from looking like everyone's dependency. No behavior change.
- Introduced `src/engine/state.js` — the engine's 31 shared mutable variables (`drops`, `score`, `bossMode`, `currentInput`, `gameSpeed`, canvas size, …) now live on a single exported `state` object that engine modules can import (ES module bindings are read-only for importers, so shared primitives need an object home). The migration of ~900 references across `script.js` was done with a scope-aware AST codemod (acorn + eslint-scope, run once then removed) so shadows and object keys were handled correctly, not a regex. Verified by the full six-engine e2e — zero behavior change. This is the foundation that makes splitting boss/input/loop into modules possible.
- Extracted the player cannon ship, its targeting laser, and the base shield visual into `src/engine/laser-ship.js` (engine split, step 2). The module owns `laser` and `playerShip` (module-private) and receives what it reads from the rest of the engine via `initShip()`: the canvas context, a canvas-size getter, a game-clock getter, the shared `fillRoundRect` helper, and a `getShieldState` getter (the shield render state — which reads boss/placement game state — stays in `script.js`). The loop/input/draw/reset call sites are unchanged; test hooks read `getLaser()`/`getPlayerShip()`. No behavior change.
- Began splitting the runtime engine into `src/engine/` modules (structural refactor, now that the test/type safety net is in place). First subsystem: `src/engine/splashes.js` owns the drop-clear splash particles (the `splashes` array is module-private) and takes the canvas context once via `initSplashes(ctx)`; the loop/input/draw pass call `create/update/draw`, resets call `resetSplashes()`. No behavior change.
- Extracted the boss bomb-spawn interval curves into pure, type-checked, unit-tested helpers in `game-core.js`: `blitzBombIntervalMs(rampUnits)` (eases 2200→700ms then log-tightens, floored 320ms — completes the Blitz ramp family alongside drop-time/speed) and `waveBombIntervalMs(load)` (tightens with simultaneous load, floored 360ms). `getBossBombIntervalMs` delegates for the blitz/wave branches (the mothership branch stays — it reads boss part state). Engine-logic track 2.4; no behavior change.
