...
## Runtime Files
- `index.html`: Game markup, operation chits, controls, canvas, input bar, touch keypad, menu/login/results/feedback links, feedback form, and overlays.
...
10. Boss mode unlocks when Mastered reaches 100%. Mastered is the percentage of current-level problems with at least 3 attempts and at least 90% current accuracy, where current accuracy heavily weights recent weighted performance with a smaller lifetime correction. Once an operation reaches 80% but is not yet boss-ready, practice generation enters a finish-level focus mode that heavily favors the remaining unmastered current-level facts. Once unlocked, a modal Boss / No boss choice pauses the board until answered. Once started, boss mode pauses normal spawning, locks the current Speed/Drops snapshot, runs Wave 1 shield endurance as a smoothed speed ramp with fixed load, then Wave 2 as a clear-gated load ladder (a round of N problems at fixed speed that only steps up after the whole round clears), then the final mothership. The mothership is a fact sheet built from the entire current-level universe (sampled, capped at 50) split across the four parts; nodes reveal in capped, answer-unique batches and empty trailing parts auto-collapse. Active challenge bombs, ship problem nodes, and node-backed boss missiles use the same input path as drops. Destroying the final core records the boss attempt and advances that operation one level.
11. Replay challenges unlock after a level boss clear. Blitz starts a shield-endurance score run for the cleared level; Wave starts a simultaneous-load score run for the cleared level; Boss replay starts directly at the mothership and records best time without advancing content level. Higher-level challenge attempts can satisfy lower-level best lookups when they are better.
...
- `progressProfile`: durable active local profile with per-operation current level, readiness, per-problem outcome history, derived pressure-tier outcome buckets, boss-completion records, legacy Blitz attempts, generalized challenge attempts for Blitz/Wave/Boss, and a bounded `sessionLog` of recent local visits. The active profile is stored inside the multi-profile store `rainMath.profiles.v1`; the legacy active-profile key `rainMath.profile.v1` is still written for compatibility. Untouched legacy `Local Player` data migrates into a profile named `david`. Boss readiness treats unseen or under-practiced current-level problems as unmastered: each problem needs at least 3 attempts and at least 90% current accuracy, and boss unlocks at 100% of that universe. Practice suggestions mix weak seen problems with unseen level problems where the operation has an enumerable universe; `getFinishLevelPracticeProblems()` identifies remaining current-level blockers once readiness reaches 80%, and `script.js` gives those blockers most practice drops until boss-ready. Pressure tiers are still derived from speed for compatibility/reporting metadata, but visible practice control is Speed plus Drops rather than a preset tier selector. Challenge bests are selected by highest score for Blitz/Wave and shortest duration for Boss; higher-level attempts can override lower-level displayed bests when they are better. Session logs separate ordinary practice stats from boss/challenge assessment stats so stressful failures do not contaminate mastery calculations. Each session also stores per-operation practice/assessment stats, challenge counters, start/end mastery snapshots, level-by-level mastery snapshots, and engaged duration from problem response times capped per item.
- `bossMode`: transient active boss, Blitz, Wave, or Boss replay state with the selected operation, target level, mode, locked Speed/Drops snapshot, phase, challenge type, shield/load counters, the Wave 2 round-spawn counter, boss parts (each holding a universe slice of revealable problem nodes), falling debris, bomb timer, stun/timer state, transition sweep state, and victory timer. It is not persisted; ordinary mastery is not updated during boss/challenge play, while completed challenge attempts update the local profile.
...
- `rainMath.welcomeSeen.v1`: small localStorage flag used only to suppress the first-visit menu after the player chooses Play or exits the Tutorial. `?test=1` suppresses the menu unless `welcome=1` is also present.
