...
- Boss readiness is universe-aware: it is the percentage of current-level problems that meet the 3-attempt/90%-current-accuracy mastery rule, with boss attempts unlocked at 100% and focused finish practice starting at 80%.
...
- Results practice suggestions blend weak seen problems for review with unseen level problems for coverage. Falling practice drops are weighted toward unmastered, low-accuracy, and under-attempted problems; above 80% current-level mastery, most practice drops for that operation come from the remaining unmastered current-level facts.
...
- During boss/challenge play the header readout (normally the session Cleared count) shows live stage progress instead: Wave 1 solved count and current speed, Wave 2 solved count and current load, and the mothership's nodes cleared. Between failed shield waves, a screen-wide bottom-to-top laser sweep represents the player's super weapon clearing the rest of the wave before the next stage begins. Level and challenge controls are locked while a boss/challenge is active. A full boss clear opens a victory summary (congratulations, the three stage results, and a Next Level button), with the accuracy grid reachable from it. A parallax starfield and a mothership that looms closer each wave reinforce the sense of approaching the boss.
