...
- Content level and practice pressure are separate mastery axes: levels measure problem scope, while speed/drop load, Blitz/Wave scores, and Worksheet time give optional fluency goals.
...
- Clearing the final mothership advances that operation to the next content level and unlocks replay challenges for the cleared level: Blitz score, Wave score, and Worksheet time. Mastery-only advancement also unlocks revisiting that level for standalone Blitz, Wave, or Worksheet attempts without forcing all three stages.
...
- Blitz, Wave, and Worksheet replay buttons appear when the selected level is already unlocked behind the current working level. Blitz is a shield-endurance score run; Wave is a simultaneous-load score run; Worksheet records time without advancing content level. Higher-level challenge results can supersede lower-level displayed bests when they are better.
...
- During boss/challenge play the header readout (normally the session Cleared count) shows live stage progress instead: Wave 1 solved count and current speed, Wave 2 solved count and current load, and the Worksheet's cleared/total count plus elapsed time. Between failed shield waves, a screen-wide bottom-to-top laser sweep represents the player's super weapon clearing the rest of the wave before the next stage begins. Operation toggles, level controls, and challenge controls are locked while a boss/challenge is active. A full boss clear opens a victory summary (congratulations, the three stage results, and a Next Level button), with the accuracy grid reachable from it. A parallax starfield and a mothership that looms closer each wave reinforce the sense of approaching the boss.
